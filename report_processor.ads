...
package Report_Processor is
   ...
   function Report_Name(for_report_number : in natural) return wide_string;
       -- For the number (starting at 1, going up to Number_of_Reports), return
   ...
   function Report_ID(for_report_name : in wide_string) return natural;
       -- Translate the report name into the report ID (i.e. into with_id).
    
private
   utf8_form     : constant string := "WCEM=8";
       -- Form for ensuring all report files are written in UTF-8 form. 
   open_chevron  : constant wide_character := wide_character'Val(16#00AB#);
   close_chevron : constant wide_character := wide_character'Val(16#00BB#);
   ...
   procedure Generate_Graphs(for_report_id : in natural);
       ...
       -- required to generate the graph.  It won't hurt as well to install the
       -- optional lcdf-typetools package, thus providing access to OpenType
       -- fonts.
   ...
   procedure Generate_LaTeX(for_report_id : in natural);
       ...
       -- Fields are referenced by column number (starting from 1) as per the
       ...
       -- Queries are nested and a query can only refer to a parent query's
       -- fields for its parameters, not any higher level (for instance, not
       -- the parent's parent query result fields).
       -- Lines in the LaTeX file can only refer to the innermost query's
       -- fields.
   ...
   procedure Display_Report(for_report_id : in natural);
       -- Execute pdflatex (or the xelatex, but you must specify this when
       -- calling Initialise) on the generated LaTeX, then display the
       -- generated PDF using a PDF viewer.  It is from that PDF viewer that
       -- the report can be printed by the user.
