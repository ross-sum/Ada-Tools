...
package body Report_Processor is
   ...
   function Report_Name(for_report_number : in natural) return wide_string is
       -- For the number (starting at 1, going up to Number_of_Reports), return
   ...
   begin
      ...
      if R_reports.Current = for_report_number then -- got there
         return Decode(UTF_8_String(Value(R_reports,1)),UTF_8);
      else  -- went past the end of data
         return "";
      end if;
   end Report_Name;
   ...
   function Report_ID(for_report_name : in wide_string) return natural is
       -- Translate the report name into the report ID (i.e. into with_id).
      use Ada.Strings.UTF_Encoding, Ada.Strings.UTF_Encoding.Wide_Strings;
      use String_Conversions;
   begin
      if Value(R_reports, 1) = Encode(for_report_name) then
         null;  -- nothing to do here
      else
         R_reports.First;
      ...
      if Value(R_reports, 1) = Encode(for_report_name) then -- got there
...
-- private
   ...
   begin
      ...
      while Has_Row(Q_reports) loop  -- execute each of the queries
         ...
         if Value(Q_reports, 1)'Length > 0 then -- a target table specified
            ...
            begin
               Error_Log.Debug_Data(at_level=> 6, 
                                    with_details=> "Run_Queries: Insert Query built '" &
                                                   Value(the_insert) & "'.");
            -- then, for each row, insert into the temporary database
               ...
               -- first, load the parameter array
                  for field_num in 1 .. num_fields loop
                     if not Is_Null(the_query, Field_Index(field_num-1)) then
                        Error_Log.Debug_Data(at_level=> 6, 
                           with_details=>"Run_Queries: Parameter " &
                                         Value_From_Wide(Decode(UTF_8_String(
                                           Value(the_query, Field_Index(field_num-1))),
                                                                UTF_8)) & ".");
               ...
               -- then execute the query to save the record in the temp DB
                  Error_Log.Debug_Data(at_level=> 6, 
                                       with_details=> "Run_Queries: Loading " &
                                                    Value(the_insert) & "...");
            ...
            Error_Log.Debug_Data(at_level=> 6, 
                                 with_details=> "Run_Queries: Execute");
   ...
   procedure Generate_Graphs(for_report_id : in natural) is
      ...
      return text is
         ...
         the_line : text;
         param_id : constant text := open_chevron & Value("PARAM:");
      begin
         if Pos(pattern => param_id, source => for_line) > 0 then
            -- Parameter(s) specified, substitute in the parameter(s)
            the_line := for_line;
            -- substitute in any parameters into «PARAM:<num>»
            while Pos(param_id, the_line) > 0 loop
               declare
                  field_pos : positive := Pos(param_id,the_line);
                  end_pos   : positive := Pos(To_Text(close_chevron),the_line);
   ...
   begin  -- Generate_Graphs
      ...
      Process_R(for_R => Disassemble(from_string => Value_From_Wide(
                                                       Decode(UTF_8_String(
                                                           Value(R_reports,4)),
                                                              UTF_8)),
   ...
   procedure Generate_LaTex(for_report_id : in natural) is
      ...
      procedure Process_Latex(for_LaTex          : text_array;
         ...
         function The_Line_Type(for_line : in text) return line_type is
            start_query : constant text := open_chevron & Value("QUERY");
            endof_query : constant text := open_chevron & Value("END QUERY");
         begin
            if Pos(pattern => start_query, source => for_line) > 0 then
               return query;
            elsif Pos(pattern => endof_query, source=>for_line) > 0 then
               return end_query;
            else
               return latex;
            end if;
         ...
         field_id     : constant text  := open_chevron & Value("FIELD:");
      begin  -- Process_Latex
         ...
         while line_num <= for_LaTex'Last loop
            ...
            case The_Line_Type(Trim(for_LaTex(line_num))) is
               when query => 
                  ...
                  query_length := Pos(To_Text(close_chevron), the_query, 1);
               ...
               when latex => 
                  ...
                  while Pos(field_id, the_line) > 0 loop
                     declare
                        use GNATCOLL.SQL_Impl;
                        field_pos : positive := Pos(field_id,the_line);
                        end_pos   : positive := Pos(To_Text(close_chevron),the_line);
                     ...
                     begin
                        ...
                        if using_params(param_num) = Null_Parameter then
                           ...
                           the_param:= Escape_The_Character('&',
                              Value_From_Wide(Decode(UTF_8_String(
                                       using_params(param_num).Image(tDB.all)),
                                                     UTF_8)));
                        end if;
                        -- pop in the parameter
                        if end_pos + 1 >= Length(the_line) then
