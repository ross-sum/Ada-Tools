...
-----------------------------------------------------------------------
   ...
   package body Generic_Binary_Trees_With_Data.Locate is
      ...
      begin
         while current_node /= null loop
            if Comparison(comparitor => current_node.all.item,
            contains => for_partial_key) then  -- found it
               return current_node.all.item;
            else
               if for_partial_key < current_node.all.item then
                  if go_down_left_if_less_than then
                     current_node := current_node.all.left;
                  else
                     current_node := current_node.all.right;
                  end if;
               else  -- ">"
                  if go_down_left_if_less_than then
                     current_node := current_node.all.right;
