...
package body Generic_Binary_Trees_With_Data is
   ...
   begin
      ...
      while current /= null loop
         last := new node'(previous, current.item, current.data,
            null, null, null);
   ...
   procedure Get(the_handle : out handle; from_the_list : in out list) is
   -- Get the current handle location and load into the handle.
   begin
      the_handle := from_the_list.current_node;
   end Get;

   function Is_Assigned(the_handle : in handle) return boolean is
   -- Return true if the handle has been assigned to a point in the tree.
   begin
      return the_handle /= null;
   end Is_Assigned;

   procedure Go_To(the_handle : in handle;
   in_the_list : in out list) is
      -- Move to the location pointed to by the handle.  If the
      -- handle is null, then don't go anywhere.
      -- Note that this procedure assumes that the handle is from
      -- this list.  It is possible to pass in a handle from a
      -- different list.  Such a possibility is not tested for
      -- because the whole purpose of this procedure is to provide
      -- a fast access mechanism.
   begin
      in_the_list.current_node := the_handle;
   end Go_To;
   ...
   function  Deliver_Data (from_the_list : in list) return D is
   -- Return a copy of the current item pointed to by the
   -- iterator.
   begin
      return from_the_list.current_node.data;  -- current item
   end Deliver_Data;
   ...
   begin
      new_node :=
         new Node'(null, the_index, the_data, null, null, null);
      if current = null then -- Empty list
         ...
         while current /= null loop
            present := current;
            if the_index < current.all.item then
         ...
         if the_index < present.all.item then
         ...
         into.current_node := new_node;  -- point to the new node
      end if;
      into.item_count := into.item_count + 1;
   end Insert;

   procedure Insert  (into : in out list; the_index : in T;
   the_data : in D; at_handle : out handle) is
      -- Perform an insert and return a handle to the node just
      -- inserted.
   begin
      Insert(into, the_index, the_data);
      at_handle := into.current_node;
   end Insert;
   ...
   procedure Delete  (from_the_list : in out list) is
      ...
      -- In certain circumstances the First and Last pointers
   ...
   begin
      if current /= null then  -- something to delete
         ...
         else  -- current.prev = null, so it must have been first
            first := current.next;  -- First in chain
            if first = null then
               last := null;   -- Empty list: last was pointing at current
            end if;
         ...
         if current.next /= null then  -- Fix backward pointer:
            ...
            if last = null then
               first := null;  -- Empty list: first was pointing at current
            end if;
         ...
         if current.right /= null then  -- Fix >= pointer
            ...
            if from_the_list.root_node = current and then
            current.left = null then
               -- we are deleting the root node
               from_the_list.root_node := current.right;
            else
         ...
         if current.left /= null then  -- Fix < pointer
            ...
            if from_the_list.root_node = current then
               -- we are deleting the root node
               from_the_list.root_node := current.left;
            else
               tree_walker := from_the_list.root_node;
               while tree_walker /= null and then
               (tree_walker.left /= current and
         ...
         if current.left = null and current.right = null then
            -- we are at a leaf child node, so null the parent
            tree_walker := from_the_list.root_node;
            while tree_walker /= null and then
            (tree_walker.left /= current and
            tree_walker.right /= current )loop
   ...
   procedure Replace (the_index : in T; for_the_list: in out list) is
      the_list : list renames for_the_list;
      the_data : D    := the_list.current_node.data;
   begin
      ...
      Insert(into => for_the_list, the_index => the_index,
         the_data => the_data);
   end Replace;

   procedure Replace (the_data : in D; for_the_list: in out list) is
      the_list  : list renames for_the_list;
   begin
      if the_list.current_node /= null then  -- exists, replace it
         the_list.current_node.all.data := the_data;
